...
class Top(Frame):
  ...
  def submitcity(self):
    ...
    try:
      ...
      self.content.weathers.city = lastcityname
      self.content.weathers.saveWeathers()
...
class Content(Frame):
  ...
  def updateWeathers(self, cityname):
      ...
      for i, wea in enumerate(self.weathers[0:6], start=0):
        self.labels[i]['text'] = wea.message
...
class LineChart(Frame):
  ...
  def drawLineChart(self):
      ...
      for i, d in enumerate(self.content.weathers.maxdegree[0:6]):
          self.pointmax.append((170 - ((d - self.minmin) * 10) - 10, 40 + i * 40))

      for i, d in enumerate(self.content.weathers.mindegree[0:6]):
          self.pointmin.append((170 - ((d - self.minmin) * 10) - 10, 40 + i * 40))
      ...
      for i in range(1, len(self.pointmax)):
          ...
          #self.chart.create_text(25, self.pointmax[i - 1][0], text=self.content.weathers[i].maxdegree, fill='red')
